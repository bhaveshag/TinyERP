    ...
    public class UpdateUserGroup : BaseIntegrationTest
    {
        ...
        public void Security_UserGroup_CreateUserGroup_ShouldThrowException_WithEmptyId()
        {
            UpdateUserGroupRequest request = new UpdateUserGroupRequest(
                Guid.Empty,
                $"new updated name {Guid.NewGuid()}",
                "desc");
            IResponseData<string> response =
                this.Connector.Put<UpdateUserGroupRequest, string>(string.Format(this.BaseUrl, request.Id), request);
            Assert.IsTrue(response.Errors.Count > 0);
            Assert.IsTrue(
                response.Errors.Any(item => item.Key == "security.addOrUpdateUserGroup.validation.idIsInvalid"));
        }
        ...
        {
            UpdateUserGroupRequest request = new UpdateUserGroupRequest(
                this.createdUserGroupResponse.Id,
                $"new updated name {Guid.NewGuid()}",
                "desc");
            IResponseData<string> response =
                this.Connector.Put<UpdateUserGroupRequest, string>(string.Format(this.BaseUrl, request.Id), request);
        ...
        private CreateUserGroupResponse CreateNewUserGroup()
        {
            CreateUserGroupRequest request = new CreateUserGroupRequest(
                $"update UserGroup name {Guid.NewGuid()}",
                "desc");
            IResponseData<CreateUserGroupResponse> response =
                this.Connector.Post<CreateUserGroupRequest, CreateUserGroupResponse>(
                    string.Format(this.BaseUrl, string.Empty),
                    request);
